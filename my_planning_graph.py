...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            ...
            if node.prenodes <= self.s_levels[level]:
                ...
                node.parents.update([p for p in self.s_levels[level] for pr in node.prenodes if p.symbol == pr.symbol])
    ...
    def add_literal_level(self, level):
        ...
        self.s_levels.append(set(
            [PgNode_s(sn.symbol, sn.is_pos) for an in self.a_levels[level-1] for sn in an.effnodes]
        ))

        for an in self.a_levels[level-1]:
            for sn in self.s_levels[level]:
                if sn in an.effnodes:
                    an.children.add(sn)
                    sn.parents.add(an)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e in node_a1.action.effect_add:
            if e in node_a2.action.precond_neg:
                return True
        for e in node_a1.action.effect_rem:
            if e in node_a2.action.precond_pos:
                return True
        for e in node_a2.action.effect_add:
            if e in node_a1.action.precond_neg:
                return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        return (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos == (not node_s2.is_pos))
